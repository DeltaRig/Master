...
configs = [
    ...
    ("10years", "1d", timedelta(days=365*10), 1),  # 245 days * 10 years 
    #("2years", "1d", timedelta(days=365*2), 1),  # 245 days * 2 years 
    #("6months", "4h", "6mo", 60),  # 22 days * 6 months 
    #("7days", "60m", "1wk", 1),
    #("24hours", "1m", "1d", (1440*0.1))  # 1440 minutes in a day 
]
...
def download_or_update(period_name, list_name, tickers, interval, keep_window, minimum_rows):
    ...
    if len(tickers) > 1: # 
        for t in tickers:
            ...
            df_t.rename(columns={df_t.columns[0]: "DateTime"}, inplace=True)
